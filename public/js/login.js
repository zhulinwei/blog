...
$(function () {
    // 表单检验规则
    var standard = {
        isNonEmpty: function isNonEmpty(dom, value, errorMsg) {
            if (value.trim() === '') {
                return {
                    dom: dom,
                    errorMsg: errorMsg
    ...
    Validator.prototype.start = function () {
        for (var i = 0, fn; fn = this.cache[i++];) {
            var msg = fn(); // 调用函数fn()后，若用户输入内容不符合表单检验规则，会返回错误信息
            if (msg) {
                // 如果出现错误信息，说明检验没有通过
                return msg;
            }
    ...
    $('#username, #password').keydown(function () {
        ...
        if (!$(this).parent().next().hasClass('hidden')) {
            (function () {
                ...
                var errorTip = $(_this).parent().next();
                // 错误提示渐变消失
